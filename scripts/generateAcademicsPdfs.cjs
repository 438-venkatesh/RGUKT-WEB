...
const PDFDocument = require(path.resolve(__dirname, '../frontend/node_modules/pdfkit'));

const outputDir = path.resolve(__dirname, '../frontend/public/docs/academics');
...
function createStyledPdf(filename, title, subtitle, documentMeta, sections) {
  ...
  doc.fillColor('#FFFFFF').fontSize(13).font('Helvetica-Bold')
    .text('RAJIV GANDHI UNIVERSITY OF KNOWLEDGE TECHNOLOGIES', 50, 18, { align: 'center' });
  doc.fontSize(9.5).font('Helvetica')
    .text('Andhra Pradesh | Constituent Campuses: Nuzvid • RK Valley • Srikakulam • Ongole', 50, 36, { align: 'center' });
  doc.fontSize(8.5).font('Helvetica-Oblique')
    .text('Catering to the Educational Needs of Gifted Rural Youth of Andhra Pradesh', 50, 50, { align: 'center' });

  doc.moveDown(2);
  doc.y = 95;

  // Title Section
  doc.fillColor('#C8102E').fontSize(13.5).font('Helvetica-Bold').text(title, { align: 'left' });
  if (subtitle) {
    doc.fillColor('#475569').fontSize(9).font('Helvetica-Bold').text(subtitle);
  }
  doc.moveDown(0.5);

  // Metadata Box
  doc.rect(50, doc.y, 495.28, 26).fill('#F1F5F9');
  doc.rect(50, doc.y, 495.28, 26).stroke('#CBD5E1');
  const metaY = doc.y + 6;
  doc.fillColor('#0A2744').fontSize(8).font('Helvetica-Bold')
    .text(`Academic Ref: ${documentMeta.ref || 'RGUKT/ACAD/2025-26'}`, 60, metaY);
  doc.text(`Effective: ${documentMeta.effective || 'Academic Year 2025-26 onwards'}`, 240, metaY);
  doc.text(`Authority: ${documentMeta.authority || 'Academic Council, RGUKT-AP'}`, 390, metaY);
  ...
  sections.forEach((sec, idx) => {
    ...
    doc.fillColor('#0A2744').fontSize(11).font('Helvetica-Bold').text(`${idx + 1}. ${sec.heading}`);
    doc.moveDown(0.25);
    ...
    if (sec.content) {
      sec.content.forEach((p) => {
        ...
        doc.fillColor('#334155').fontSize(9).font('Helvetica').text(p, {
          align: 'justify',
          lineGap: 2.5,
        });
        doc.moveDown(0.3);
    ...
    if (sec.bullets) {
      sec.bullets.forEach((b) => {
        ...
        doc.fillColor('#C8102E').fontSize(9).font('Helvetica-Bold').text('• ', { continued: true });
        if (typeof b === 'string') {
          doc.fillColor('#334155').font('Helvetica').text(b, { lineGap: 2 });
        } else {
          doc.fillColor('#0A2744').font('Helvetica-Bold').text(`${b.title}: `, { continued: true });
          doc.fillColor('#334155').font('Helvetica').text(b.desc, { lineGap: 2 });
        }
        doc.moveDown(0.18);
      });
      doc.moveDown(0.3);
...
async function generateAllPdfs() {
  console.log('Generating official RGUKT-AP PDF documents with authentic matter...');
  ...
  console.log('ALL official RGUKT-AP PDF documents generated successfully with authentic data!');
}
