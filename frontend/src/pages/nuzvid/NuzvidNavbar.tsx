...
export default function NuzvidNavbar() {
  const scrolled = useHeaderScroll(50);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [openMenu, setOpenMenu] = useState<string | null>(null);
  const location = useLocation();
  const isHome = location.pathname === '/nuzvid' || location.pathname === '/nuzvid/';
  ...
  const menuActive = (href: string) => {
    ...
    return path !== '/nuzvid' && location.pathname.startsWith(path + '/');
  };

  return (
    <>
      <a href="#nuzvid-main" className="skip-link">Skip to content</a>

      <header className={`ap-header${isHome ? ' overlay' : ''}${solidNav ? ' solid' : ''}${scrolled ? ' compact' : ''}`}>
        <nav className={`ap-nav${solidNav ? ' ap-nav-solid' : ''}`} aria-label="Nuzvid campus navigation">
          <div className={`ap-nav-inner${solidNav ? ' has-bulge' : ''}`}>

            {solidNav && (
              <Link to="/nuzvid" className="ap-nav-bulge" aria-label="RGUKT Nuzvid Home">
      ...
      {mobileOpen && (
        <div className="ap-mobile-overlay" role="dialog" aria-modal="true" aria-label="Mobile navigation">
          ...
          <nav className="ap-mobile-nav">
            {getNuzvidMobileNavLinks().map(item => (
              <NavHref
                key={item.label + item.href}
                item={item}
